...
class GateSumAlternate(GateSum):
  ...
  def __init__(self):
    ...
    self._overflow_gate = GateConsensusAlternate()
...
class GateSum3(GateDiadic):
  ...
  def __init__(self):
    ...
    self._overflow_gate = GateSumAlternate()
...
class GateConsensusAlternate(GateDiadic):
  """
  Alternate version of GateSum whose logic is completely built off of other
  gates.
  Key:
  ...
  def __init__(self):
    ...
    # 0
    zero = GateNeutral()
    zero.SetInputWire(wires[0])
    zero.SetOutputWire(wires[10])

    # A = ~(a = -1) ^ 0 (wire 4)
    lowA = GateIsLow()
    lowA.SetInputWire(wires[0])
    lowA.SetOutputWire(wires[2])
    negateLowA = GateNegate()
    negateLowA.SetInputWire(wires[2])
    negateLowA.SetOutputWire(wires[3])
    lowAandZero = GateAnd()
    lowAandZero.SetInputWire1(wires[3])
    lowAandZero.SetInputWire2(wires[10])
    lowAandZero.SetOutputWire(wires[4])

    # B = (a ^ b ) (wire 5)
    AandB = GateAnd()
    AandB.SetInputWire1(wires[0])
    AandB.SetInputWire2(wires[1])
    AandB.SetOutputWire(wires[5])

    # C = ~(b = -1) ^ 0 (wire 6)
    lowB = GateIsLow()
    lowB.SetInputWire(wires[1])
    lowB.SetOutputWire(wires[7])
    negateLowB = GateNegate()
    negateLowB.SetInputWire(wires[7])
    negateLowB.SetOutputWire(wires[8])
    lowBandZero = GateAnd()
    lowBandZero.SetInputWire1(wires[8])
    lowBandZero.SetInputWire2(wires[10])
    lowBandZero.SetOutputWire(wires[6])

    # A v B
    or1 = GateOr()
    or1.SetInputWire1(wires[4])
    or1.SetInputWire2(wires[5])
    or1.SetOutputWire(wires[9])

    # (A v B) v C
    self._output_gate = GateOr()
    self._output_gate.SetInputWire1(wires[9])
    self._output_gate.SetInputWire2(wires[6])
